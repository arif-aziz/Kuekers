	...
	<div id="p-large" class="wr-nitro-carousel images">
		<?php
			...
			if ( has_post_thumbnail() ) {
				...
				echo apply_filters( 'woocommerce_single_product_image_html', sprintf( '<a ' . $lightbox . ' href="%s" itemprop="image" data-type="image-gallery-thumb-highlights" title="%s" data-lightbox="nivo" ' . $gallery . '>%s</a>', $image_link, $image_caption, $image ), $post->ID );
		...
		<div id="p-thumb" class="fc fcc fcw">
			<?php
				if ( has_post_thumbnail() ) {
					...
					echo apply_filters( 'woocommerce_single_product_image_html', sprintf( '<div class="nav-item" data-type="image-gallery-thumb-highlights">%s</div>', $image ), $post->ID );
				} else {
	...
	(function($) {
		...
		$(document).ready(function() {
			...
			function image_owlCarousel() {
				...
				$( '#p-thumb' ).css( 'max-height', max_height / 2 );
			}
