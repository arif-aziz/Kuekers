...
class WR_Nitro_Pluggable_WooCommerce {
	...
	public static function initialize() {
		...
		// Creat action with ajax
		add_action( 'wp_loaded', array( __CLASS__, 'action_cart_ajax' ), 999999999 );
		...
		?>
			...
			<div class="variations_button clearfix">
				...
				<?php
					...
					$add_to_cart_button = '<button type="submit" class="' . ( ! $check_gravityforms ? 'wr_single_add_to_cart_ajax' : NULL ) . ' variation single_add_to_cart_button wr_add_to_cart_button button alt btr-50 db pdl20 pdr20 fl mgr10 mgt10 br-3"><i class="nitro-icon-' . esc_attr( $icons ) . '-cart mgr10"></i>' . esc_html( $product->single_add_to_cart_text() ) . '</button>';
	...
	public static function add_to_cart_ajax() {
		...
		self::$cart_action_notice = '<script id="tp-notice-html" type="text/html">' . json_encode( $data ) . '</script>';

		add_action( 'wp_footer', array( __CLASS__, 'print_cart_notice' ) );
	}
