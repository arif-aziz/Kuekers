...
<div class="product__wrap product-btn-left pr oh <?php if ( $product->get_rating_html() ) echo 'has-rating' ?>">
		...
		<?php if ( ! ( ! $product->get_price() && $wr_catalog_mode ) && $wr_show_price ) : ?>
			<div class="product__action pa body_bg">
				<div class="product__action-inner icon_color fl">
					<?php
						// Add to cart button
						if ( ( $wr_nitro_options['wc_buynow_btn'] && ! $wr_nitro_options['wc_disable_btn_atc'] ) || ! $wr_nitro_options['wc_buynow_btn'] || ( $wr_nitro_options['wc_buynow_btn'] && $wr_nitro_options['wc_disable_btn_atc'] && $product->product_type != 'simple' ) && $booster_pop != 'yes' ) {
							wc_get_template( 'loop/add-to-cart.php' );
						}

						// Quick buy button
						if ( $wr_nitro_options['wc_buynow_btn'] && $product->product_type == 'simple' && ! $wr_catalog_mode && $product->is_in_stock() && ! WR_Nitro_Helper::check_gravityforms( $post->ID ) && ! WR_Nitro_Helper::yith_wc_product_add_ons( $post->ID ) && ! WR_Nitro_Helper::wc_measurement_price_calculator( $post->ID ) ) {
	...
	<div class="product__title pa ts-05 tc">
		<h5 class="mg0"><a class="hover-primary" href="<?php esc_url( the_permalink() ); ?>" title="<?php echo wp_kses( get_the_title(), '' ); ?>"><?php the_title(); ?></a></h5>
