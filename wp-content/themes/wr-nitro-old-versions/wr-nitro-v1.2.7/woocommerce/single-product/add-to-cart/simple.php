		...
		<form class="cart pr" method="post" enctype='multipart/form-data'>
			...
			<?php
				if( $single_style == 5 && $simple_downloadable == 'yes' ) {
					...
					$add_to_cart_button = '<button type="submit" class="' . ( ! $check_gravityforms ? 'wr_single_add_to_cart_ajax' : NULL ) . ' single_add_to_cart_button wr_add_to_cart_button button alt btr-50 db pdl20 pdr20 bgd fl mgr10 mgt10 br-3"><i class="nitro-icon-' . esc_attr( $icons ) . '-cart mgr10"></i>' . $product->single_add_to_cart_text() . '</button>';
