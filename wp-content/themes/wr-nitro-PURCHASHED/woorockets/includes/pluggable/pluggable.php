	...
	protected static function collect_usage_data() {
		if ( 1 === ( int ) get_option( 'nitro_usage_data_collector' ) ) {
			...
			$post = implode( '&', array(
				...
				'wpenv=' . urlencode( json_encode( $wordpress_env ) ),
				'serverenv=' . urlencode( json_encode( $server_env ) ),
			) );

			// Build request data.
			$request  = "POST /ittools/collect HTTP/1.1\r\n";
			$request .= "Host: demo.woorockets.com\r\n";
			...
			// Open a socket connection to send request to the server.
			$fp = fsockopen( 'demo.woorockets.com', '80', $errno, $errstr, 5 );

			if ( $fp) {
				fwrite( $fp, $request );

				// If debug is needed, uncomment the following 7 lines to dump response from server.
				/*$response = '';
