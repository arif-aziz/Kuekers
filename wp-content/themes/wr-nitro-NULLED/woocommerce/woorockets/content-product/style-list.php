	...
	<?php if ( ! $wr_catalog_mode ) : ?>
		<div class="product__action fl mgt20">
			<?php
				...
				// Quick buy button
				if ( $wr_nitro_options['wc_buynow_btn'] && $product->is_type( 'simple' ) && ! WR_Nitro_Helper::check_gravityforms( $post->ID ) && ! WR_Nitro_Helper::yith_wc_product_add_ons( $post->ID ) && ! WR_Nitro_Helper::wc_measurement_price_calculator( $post->ID ) ) {
